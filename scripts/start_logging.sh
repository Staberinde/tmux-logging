...
pipe_pane_ansifilter() {
	tmux pipe-pane "exec cat - | ansifilter | $encode >> $FILE"
}
...
pipe_pane_sed_osx() {
	...
	tmux pipe-pane "exec cat - | sed -E \"s/$ansi_codes_osx//g\" | $encode >> $FILE"
}

pipe_pane_sed() {
	local ansi_codes="(\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]|)"
	tmux pipe-pane "exec cat - | sed -r 's/$ansi_codes//g' | $encode >> $FILE"
}
